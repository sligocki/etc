...
/// RMonus: y ∸ x  (reversed monus; first arg is subtracted from second)
...
/// Shift: x · 2^k  (first arg k is the shift amount, second arg x is the value)
...
/// RMonusOdd: y ∸ (2x + 1)  (first arg x, second arg y)
...
/// Div2k: ⌊x / 2^k⌋  (first arg k, second arg x)
...
/// SafeBlock: x · 2^k ∸ Sgn(k)  (first arg k, second arg x)
...
/// Bit: the k-th bit of x  (first arg k, second arg x)
/// C(Mod2, Div2k)
/// Arity: 2, Size: 28
pub fn bit() -> Grf {
...
/// PopK: length of the lowest run of 1-bits  (= min k s.t. bit k of x = 0)
/// M(Bit)
/// Arity: 1, Size: 29
pub fn pop_k() -> Grf {
    Grf::Min(Box::new(bit()))
...
/// AppendN: apply the SafeBlock transform N times starting from x
///   AppendN(0, k, x)   = x          [P(2,2) selects x as the 2nd rest-arg]
///   AppendN(n+1, k, x) = SafeBlock(AppendN(n, k, x), k)
///                       = k * 2^AppendN(n,k,x) ∸ Sgn(AppendN(n,k,x))
...
/// AckStep: full state mutation
///   AckStep(a, b) = Div2(AppendN(a, b, PopK(b)))
...
/// StateLoop: iterate AckStep n times
///   StateLoop(0, x)   = x
///   StateLoop(n+1, x) = AckStep(n, StateLoop(n, x))
/// R(P(1,1), C(AckStep, P(3,1), P(3,2)))
/// Arity: 2, Size: 85
pub fn state_loop() -> Grf {
    Grf::Rec(
        Box::new(Grf::Proj(1, 1)),
        Box::new(Grf::comp(
            ack_step(),
...
/// AckWorm: A version of Hydra game/Goodstein sequence
/// Arity: 1, Size: 86
pub fn ack_worm() -> Grf {
    Grf::Min(Box::new(state_loop()))
}
...
mod tests {
    ...
    fn test_state_loop() {
        // state_loop(0, x) = x
        // state_loop(n+1, x) = ack_step(n, state_loop(n, x))
        let f = state_loop();
        for x in 0u64..=5 {
    ...
    fn test_ack_worm_small() {
        // ack_worm(x) = min{n : state_loop(n,x) = 0}
        let f = ack_worm();
